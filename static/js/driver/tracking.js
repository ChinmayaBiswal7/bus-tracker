...
let currentServiceType = 'HOSTEL'; // NEW STATE
...
function handleGPS(pos, busNo, stage) {
    ...
    if (socket.connected) {
        socket.emit('driver_update', {
            ...
            type: currentServiceType, // NEW FIELD
            driver_name: document.getElementById('main-body')?.getAttribute('data-user-name') || 'Driver' // FETCH NAME
        });
    } else {
        if (statStatus) {
...
export function setServiceType(type) {
    ...
    console.log(`[DRIVER] Service Type set to: ${type}`);

    // Update UI
    const btns = {
        'HOSTEL': document.getElementById('btn-type-hostel'),
        'DAY_SCHOLAR': document.getElementById('btn-type-day')
    };
    ...
    // Highlight
    if (btns[type]) {
        btns[type].classList.remove('bg-slate-800', 'border-slate-700', 'text-slate-400');
        btns[type].classList.add('bg-blue-600', 'border-blue-500', 'text-white');
    }
}
